...
impl App {
    ...
    pub async fn run_loop(&mut self) {
        loop {
            ...
            match self.mode {
                AppMode::Driving => self.update_and_render_driving(dt, current_time),
                AppMode::TrackEditor => self.update_and_render_editor(dt),
            }
    ...
    fn update_and_render_driving(&mut self, dt: f32, current_time: f64) {
        if is_key_pressed(KeyCode::R) {
            self.reset_car_to_grid();
        }
    ...
    fn reset_car_to_grid(&mut self) {
        let (spawn_pos, spawn_heading) = self.track.start_grid_config.get_anchor_transform(0);
    ...
    fn update_and_render_editor(&mut self, dt: f32) {
        ...
        let world_mouse = self.camera_target + (screen_mouse - screen_center) / self.editor_zoom;
    ...
    fn exit_editor_to_driving(&mut self) {
        self.track
            .rebuild_mesh(5, self.track_texture.as_ref(), self.wall_texture.as_ref());
        self.mode = AppMode::Driving;

        let (spawn_pos, heading) = self.track.start_grid_config.get_anchor_transform(0);
